...
Скачать `Obsidian` можно с официального сайта: [Download - Obsidian](https://obsidian.md/download)

Когда Вы решитесь освоить `Obsidian`, советую пройти гайд от самих разработчиков. Цель гайда - научить Вас работать в `markdown` и показать некоторые уникальные фишки `Obsidian`. Если Вас интересует что-то другое касательно `Obsidian`, то у Вас всегда есть возможность посетить официальный сайт - [Home - Obsidian Help](https://obsidian.md/help/) или просто [Obsidian - Sharpen your thinking](https://obsidian.md/).

Для получения полного гайда Вам потребуется либо в меню `Manage Vaults` найти `Sandbox Vault`, либо если там её нет, то открыть любую папку как `Vault`, а затем снизу в панели настроек найти знак вопроса, кликнув на который, у Вас откроется меню пунктом `Sandbox Vault`.

>[!tip]
>Как оказалось, эти инструкции - не полный и, признаться честно, не лучший разбор возможностей `Obsidian` - уж больно простым языком всё рассказывается. Совершенно случайно мне попался разбор куда лучше, так сказать, уже для взрослых, но предназначенный изначально для обучения `gemini` работе с приложением. Этот разбор я скачал с сайта плагина [Gemini Scribe](https://github.com/allenhutchison/obsidian-gemini/). В папке [bundled-skills](https://github.com/allenhutchison/obsidian-gemini/blob/master/prompts/bundled-skills) есть несколько заметок, но для человека полезна только эта четвёрка:
>
> | Тема                       | Ссылка                  |
> | -------------------------- | ----------------------- |
...
>
> Для изучения `Obsidian` достаточно прочитать 1-ый файл, [obsidian-markdown](guides_for_ai/obsidian-markdown.md), в котором рассказывается самый, что называется, сок. Остальное - уже на любителя.
...
Не знаю, будете ли Вы сохранять свою `Vault` в `Git`/`GitHub`/`GitLab`, но в любом случае Вы сможете отслеживать версии, причём эта функция встроена по умолчанию! Если Вы перейдёте по `Settings` -> `Core Plugins` -> `File Recovery`, то увидите, что каждые 5 минут создаётся снимок Ваших `md`-файлов. Эти снимки хранятся неделю и Вы можете посмотреть каждый из них для любой заметки, кликнув на `View`. Очень полезная фишка, которая спасает в важные моменты (особенно, когда удаляете файл из системной корзины ноутбука 😅)
...
Помимо самого приложения [obsidianmd/obsidian-releases](https://github.com/obsidianmd/obsidian-releases) в репозитории компании [Obsidian.md](https://github.com/obsidianmd) есть и другой немалый проект - [obsidianmd/obsidian-clipper](https://github.com/obsidianmd/obsidian-clipper). Перейтя по ссылке, вам откроется `README.md`, в котором будет пару слов о том, что вообще это за репа, и будет выбор магазина для установки расширения в браузер. Из репозиториев Obsidian ещё есть [obsidianmd/obsidian-api](https://github.com/obsidianmd/obsidian-api), но его применение для меня пока остаётся загадкой - только если вы строите второй мозг по Карпатому (что, как мне кажется, дело бессмысленное) или что-то в этом духе.

Очень рекомендую это расширение, если вы часто видете страницу, содержание которой хочется сохранить себе на память - будь то важная статья о версионировании в современной индустрии или маленькая автобиография футболиста. Расширение позволит скачивать в формате `md` весь текст сайта, поддерживая разделения по топикам и вписывая в начало заметки ссылку на сайт.
...
Спустя почти полгода активного пользования Obsidian для себя я решил разделить все свои файлы: вести заметки в Obsidian и синхронизировать через GitHub (3-им способом из вышеперечисленного), а все файлы оставить в гугл диске. О том, почему использовать гитхаб для всех-всех своих файлов, я рассказывать тут не стану, просто скажу, что 1) отправлять файлы более 100 Мб в гитхаб нельзя 2) клонировать репозиторий после 2 Гб становится задачкой со звёздочкой (или даже двумя звёздочками) 3) будет просто полезно разделить ваши файлы на `для работы/учёбы/IT` и на `Личное`.
